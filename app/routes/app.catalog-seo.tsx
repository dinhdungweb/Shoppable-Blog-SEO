...
import { Badge, Banner, BlockStack, Box, Button, Card, Icon, IndexTable, InlineGrid, InlineStack, Modal, Page, Pagination, Select, Tabs, Text, TextField, Thumbnail } from "@shopify/polaris";
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const [rows, filteredTotal, total, aggregate, needsAttention, issueRows, job] = await Promise.all([
    ...
    prisma.resourceSEO.findMany({ where: { ...baseWhere, issueCount: { gt: 0 } }, select: { issues: true }, orderBy: { issueCount: "desc" }, take: 5000 }),
  ...
  const allIssues = issueRows.flatMap((row) => parseIssues(row.issues));
  const grouped = allIssues.reduce((groups, issue) => {
    const current = groups.get(issue.type);
    groups.set(issue.type, { label: issue.label, count: (current?.count || 0) + 1, impact: issue.impact });
    return groups;
  }, new Map<string, { label: string; count: number; impact: CatalogSeoIssue["impact"] }>());
  const issueGroups = [...grouped.values()].sort((a, b) => b.count - a.count).slice(0, 6);
  ...
  return json({ resourceType, page, total, filteredTotal, totalPages, query, status, averageScore: Math.round(aggregate._avg.seoScore || 0), needsAttention, totalIssues: aggregate._sum.issueCount || 0, issueGroups, rows: data, job: job ? { status: job.status, phase: job.phase } : null });
};
...
export default function CatalogSeoPage() {
  ...
  return <Page fullWidth>
    ...
    <BlockStack gap="500">
      <InlineStack align="space-between" blockAlign="end" gap="300"><BlockStack gap="100"><Text as="h1" variant="headingXl" fontWeight="bold">{typeLabel} SEO</Text><Text as="p" tone="subdued">Review Shopify search listings, descriptions, URL handles and image SEO.</Text></BlockStack><Button variant="primary" loading={isScanning} onClick={runScan}>Run SEO scan</Button></InlineStack>
      <div className="bp-seo-scope-tabs"><Tabs tabs={SCOPE_TABS} selected={data.resourceType === "product" ? 1 : 2} onSelect={switchScope} /></div>
      ...
      {data.issueGroups.length > 0 && <Card><BlockStack gap="300"><InlineStack align="space-between"><Text as="h2" variant="headingMd">Top issues</Text><Text as="span" variant="bodySm" tone="subdued">Across all scanned {typeLabel.toLowerCase()}</Text></InlineStack><div className="bp-catalog-issue-grid">{data.issueGroups.map((issue) => <div className="bp-catalog-issue-summary" key={issue.label}><span className={`bp-catalog-impact is-${issue.impact.toLowerCase()}`} /><div><Text as="p" fontWeight="semibold">{issue.label}</Text><Text as="p" variant="bodySm" tone="subdued">{issue.count} affected</Text></div></div>)}</div></BlockStack></Card>}
      <Card padding="0">
